...
static void uf_send_gratuitous_arp(unifi_priv_t *priv, CsrUint16 interfaceTag)
{
    netInterface_priv_t *interfacePriv = priv->interfacePriv[interfaceTag];
    ...
    CsrInt8 protection;
    int r;
    static const CsrUint8 arp_req[36] = {
 ...
 * ---------------------------------------------------------------------------
 */
static int configure_data_port(unifi_priv_t               *priv,
...
void CsrWifiRouterCtrlWifiOnReqHandler(void *drvpriv, CsrWifiFsmEvent *msg)
{
    ...
    CsrWifiRouterCtrlWifiOnIndSend(msg->source, 0, CSR_RESULT_SUCCESS, versions, 0);
 ...
 *      CsrWifiRouterCtrlWifiOffRspHandler().
 */
static void wifi_off(unifi_priv_t *priv)
{
    int power_off;
    int priv_instance;
    int i;
...
void CsrWifiRouterCtrlWifiOnResHandler(void *drvpriv, CsrWifiFsmEvent *msg)
{
...
#if defined(CSR_WIFI_SPLIT_PATCH) && !defined(CSR_WIFI_FW_RELOAD)
...
#if defined(ANDROID_BUILD) && !defined(CSR_WIFI_FW_RELOAD)
    ...
    {
        /* Acknowledge the CsrWifiRouterCtrlWifiOnReq now */
        unifi_error(priv, "%s : WIFI ON failed : %d\n", __FUNCTION__, res->status);
        CsrWifiRouterCtrlWifiOnCfmSend(msg->source, res->clientData, CSR_RESULT_FAILURE);
    }
}
...
static int _sys_packet_req(unifi_priv_t *priv, const CSR_SIGNAL *signal,
...
#ifdef __powerpc__
     signal.SignalPrimitiveHeader.SenderProcessId = htons((priv->sme_cli->sender_id & 0xff00) | (unsigned int)msg->source);
#else
...
void CsrWifiRouterCtrlModeSetReqHandler(void *drvpriv, CsrWifiFsmEvent *msg)
{
    ...
    if (req->interfaceTag < CSR_WIFI_NUM_INTERFACES)
    {
...
#ifdef CSR_WIFI_FW_RELOAD
        /* Do not allow mode change if proper patch in not set by iwprivsfwreload */
        if (req->mode != 0)
        {
            if ((CSR_WIFI_HIP_IS_AP_FW(req->mode) && interfacePriv->patchMode == 0) ||
                (!CSR_WIFI_HIP_IS_AP_FW(req->mode) && interfacePriv->patchMode == 1))
            {
                unifi_error(priv, "%s : mode change not allowed (req = %d ; patch = %d)\n",
                            __FUNCTION__, req->mode, interfacePriv->patchMode);
        ...
        if ((CSR_WIFI_HIP_IS_AP_FW(req->mode) && !interfacePriv->patchMode) ||
        		(CSR_WIFI_HIP_IS_STA_FW(req->mode) && interfacePriv->patchMode))
        {
            CsrWifiRouterCtrlVersions versions;
            int r;
            ...
            interfacePriv->patchMode = CSR_WIFI_HIP_IS_AP_FW(req->mode);
            r = uf_request_firmware_files(priv, UNIFI_FW_STA);
            if (r)
            {
               	...
               	CsrWifiRouterCtrlModeSetCfmSend(msg->source, req->clientData, req->interfaceTag,
                    	                            req->mode, CSR_RESULT_FAILURE);
        	      interfacePriv->patchMode = old_mode;
               	return;
            }
...
void uf_send_ba_err_wq(struct work_struct *work)
{
                               ...
                               )
{
    ...
    unifi_trace(priv, UDBG1, "%s: ba session with peer= (%02x:%02x:%02x:%02x:%02x:%02x)\n", __FUNCTION__,
                  macAddress.a[0], macAddress.a[1], macAddress.a[2],
                  macAddress.a[3], macAddress.a[4], macAddress.a[5]);

    unifi_trace(priv, UDBG1, "%s: ba session for tID=%d timeout=%d role=%d wind_size=%d start_sn=%d\n", __FUNCTION__,
                  tID,
                  timeout,
                  role,
                  wind_size,
    ...
    else if (role == CSR_WIFI_ROUTER_CTRL_BLOCK_ACK_RECIPIENT)
    {
        ...
        {
            unifi_trace(priv, UDBG1, "blockack_session_start: Controlled port not opened, Reject BA request\n");
            return FALSE;
        }
